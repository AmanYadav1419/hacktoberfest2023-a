...
def isSafe(board, row, col, ch, top, left, bottom, right):
    ...
    if (
        row >= 1
        and board[row - 1][col] == ch
        or (col + 1 < N and board[row][col + 1] == ch)
        or (row + 1 < M and board[row + 1][col] == ch)
        or col >= 1
        and board[row][col - 1] == ch
    ):
        return False
...
def validateConfiguration(board, top, left, bottom, right):
    ...
    return not any(
        right[j] != -1 and countInRow(board, '-', j) != right[j]
        for j in range(M)
    )
...
def solveMagnetPuzzle(board, row, col, top, left, bottom, right, rules):
    ...
    return bool(
        solveMagnetPuzzle(board, row, col + 1, top, left, bottom, right, rules)
    )
...
def magnetPuzzle(top, left, bottom, right, rules):
    ...
    board = [['X' for _ in range(N)] for _ in range(M)]
